...
//        im.saveJPG("out_local_thresholding");
        ...
        displayGrayscaleImage(&im, ui->outputImageLabel);
        ui->sigmaCannySlider->setValue(1);
        ui->thLowCannySlider->setValue(0.3 * 255);
        ui->thHighCannySlider->setValue(0.9 * 255);
    }
}
...
void MainWindow::on_thHighCannySlider_valueChanged(int val) {
    ...
    auto im = cannyEdgeDetector(grayImage, ui->sigmaCannySlider->value(), val / 255., ui->thLowCannySlider->value()/255.);
    displayGrayscaleImage(&im, ui->outputImageLabel);
}
...
void MainWindow::on_thLowCannySlider_valueChanged(int val) {
    ...
    auto grayImage = Image(inputImage);
    auto im = cannyEdgeDetector(grayImage, ui->sigmaCannySlider->value(),ui->thHighCannySlider->value()/255. , val / 255.);
    displayGrayscaleImage(&im, ui->outputImageLabel);
}
void MainWindow::on_sigmaCannySlider_valueChanged(int val) {
    ui->sigmaCannyValLabel->setNum(val);
    auto grayImage = Image(inputImage);
    auto im = cannyEdgeDetector(grayImage, val, ui->thHighCannySlider->value()/255., ui->thLowCannySlider->value()/255.);
    displayGrayscaleImage(&im, ui->outputImageLabel);
}
